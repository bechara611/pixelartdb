...
const App = () => {
  ...
  useEffect(() => {
    ...
    setPixels(Array.from({ length: divsAloAlto * divsAlOancho }, () => { return { active: false, color: 'transparent' } }))

  }, [])

  const funcionArray = (index) => {
    let nuevoArray = [...pixels]
    nuevoArray[index] = { active: true, color: 'transparent' }
  ...
  const onMouseOver2 = (index) => {
    if (isSelecting) {
      ...
      pixelsupdated[index] = { ...pixelsupdated[index], active: true }
